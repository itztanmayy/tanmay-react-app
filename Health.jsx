...
function Health() {
  ...
  const [healthListings, setHealthListings] = useState([]);
  ...
  useEffect(() => {
    ...
    if (savedListings) {
      ...
      // Filter listings for health category
      const healthItems = allListings.filter(
        (item) => item.category === "Health" || item.category === "health"
      );
      setHealthListings(healthItems);
    }
    ...
    <div className="ebay-homepage">
      ...
      <nav className="main-nav">
        <div className="main-nav-container">
          ...
          <Link to="/health" className="nav-item active">Health</Link>
        </div>
      </nav>
      ...
      <section className="hero-banner">
        <div className="banner-content">
          <h1>Health</h1>
          <p>Shop for wellness and health products.</p>
        </div>
      </section>
      ...
      <section className="fresh-recommendations">
        ...
        <div className="product-grid">
          {healthListings.length > 0 ? (
            healthListings.map((product) => (
            ...
            <div className="no-listings-message">
              <p>No health listings available yet. Be the first to sell health products!</p>
              <button onClick={handleSellClick} className="selll-button">Sell Health Products Now</button>
            </div>
          )}
        </div>
      </section>
