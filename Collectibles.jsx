...
function Collectibles() {
  ...
  useEffect(() => {
    ...
    if (savedListings) {
      ...
      // Filter listings for collectibles category
      const collectiblesItems = allListings.filter(
        (item) => 
          item.category === "Collectibles and Art" || 
          item.category === "collectibles" || 
          item.category === "Collectibles" || 
          item.category === "Art" ||
          item.category === "art"
      );
      setCollectiblesListings(collectiblesItems);
    }
    ...
    <div className="ebay-homepage">
      ...
      <nav className="main-nav">
        <div className="main-nav-container">
          ...
          <Link to="/collectibles" className="nav-item active">Collectibles and Art</Link>
      ...
      <section className="hero-banner">
        <div className="banner-content">
          <h1>Collectibles and Art</h1>
          <p>Find rare collectibles and unique art pieces.</p>
        </div>
      </section>

      <section className="fresh-recommendations">
        <h2>Collectibles and Art Products</h2>
        <div className="product-grid">
          {collectiblesListings.length > 0 ? (
            collectiblesListings.map((product) => (
            ...
            <div className="no-listings-message">
              <p>No collectibles or art listings available yet. Be the first to sell collectibles or art!</p>
              <button onClick={handleSellClick} className="selll-button">Sell Collectibles Now</button>
            </div>
          )}
        </div>
      </section>
