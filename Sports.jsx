...
function Sports() {
  ...
  const [sportsListings, setSportsListings] = useState([]);
  ...
  useEffect(() => {
    ...
    if (savedListings) {
      ...
      // Filter listings for sports category
      const sportsItems = allListings.filter(
        (item) => item.category === "Sports" || item.category === "sports"
      );
      setSportsListings(sportsItems);
    }
    ...
    <div className="ebay-homepage">
      ...
      <nav className="main-nav">
        <div className="main-nav-container">
          ...
          <Link to="/sports" className="nav-item active">Sports</Link>
      ...
      <section className="hero-banner">
        <div className="banner-content">
          <h1>Sports</h1>
          <p>Get active with sports equipment and athletic gear.</p>
        </div>
      </section>
        ...
        <div className="product-grid">
          {sportsListings.length > 0 ? (
            ...
            <div className="no-listings-message">
              <p>No sports listings available yet. Be the first to sell sports items!</p>
              <button onClick={handleSellClick} className="selll-button">Sell Sports Items Now</button>
            </div>
          )}
        </div>
      </section>
